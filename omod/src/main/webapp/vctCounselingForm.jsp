...
<div class="box">
	...
	<table id="list_data">
		...
		<c:forEach items="${personIds}" var="pId" varStatus="status">
			<tr class="${status.count%2!=0?'even':''}">
				...
				<td class="rowValue" style="width: 250px;">
					...
					<input type="hidden" id="personId_${pId}" name="personId_${pId}"/><span id="personName_${pId}"></span><input class="smallButton" type='button' value='Select' name="popup_${pId}" onclick="createPopUp(this,'<spring:message code='vcttrac.registration.findClient'/>','<spring:message code='vcttrac.registration.clientName'/>/<spring:message code='vcttrac.registration.codeclient'/>');" id="bt_popup_${pId}"/>
				
					<span id="patientError_${pId}"></span>
				</td>
				<td class="rowValue"><select name="whyTesting_${pId}" id="whyTestingId_${pId}">
						<option value="0">--</option>
						<c:forEach items="${reasonOfHivTest}" var="reason">
							<option value="${reason.key}">${reason.value}</option>
						</c:forEach>
					</select>
					<span id="whyTestingError_${pId}"></span>
				</td>
				<td class="rowValue"><select name="refereDuService_${pId}" id="refereDuServiceId_${pId}">
						<option value="0">--</option>
						<c:forEach items="${programOrderers}" var="program">
							<option value="${program.key}">${program.value}</option>
						</c:forEach>
					</select>
					<span id="refereDuServiceError_${pId}"></span>
				</td>
				<td class="rowValue"><textarea title="" onblur="setTitle(this);" name="comment_${pId}" cols="60" rows="2"></textarea></td>
			</tr>
		</c:forEach>
	</table>
</div>
